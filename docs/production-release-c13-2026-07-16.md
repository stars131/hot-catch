...
- 生产边界修复提交：`35bcd8a`（未配置模型时禁止 `local-template` 假成功）。
...
- 公网入口：Cloudflare A 记录指向 `65.49.231.53`，该地址是既有中转/反向代理，不是应用主机。
- 应用主机：`12.22.163.133`
- 实际链路：Cloudflare → `65.49.231.53` → `12.22.163.133:80` Nginx → `127.0.0.1:3040`。
- Web：`min-xingji-prod.service`，监听 `127.0.0.1:3040`
- Worker：`min-xingji-worker-prod.service`，使用独立 Redis DB。
- 当前发布：`/opt/min-xingji/releases/20260715-182356-c13-prod/app`
- 上一发布：`/opt/min-xingji/releases/20260715-235645-prod/app`
...
- Cloudflare `creat-mcn.wlwl-tools.com` A 记录保持 `65.49.231.53`，代理开启、TTL 自动；不得改为直连 12，因为会绕过既有中转层。
...
- 管理员邀请：`2952932090@qq.com` 已完成首次登录，当前为 `accepted`；Resend 投递状态为 `delivered`。

## 公网链路与回滚演练

- 真实日志已证明 `65.49.231.53` 的请求进入 `min-xingji-production.access.log`，并由生产 Nginx 转发到 3040。
- 曾将 A 记录改为直连 `12.22.163.133` 做演练；公网返回 Cloudflare 522 后立即恢复 `65.49.231.53`，健康检查随即恢复 ready。
- 为直连诊断临时创建的 `creat-mcn-origin-port-80` Origin Rule 已删除，Cloudflare 恢复原始配置。
- 443 继续由 Xray 监听，未修改其配置；其他 Nginx 站点与服务未改动。

## 生产验收

- 公网首页正确进入邀请制登录页，`/api/health` 为 ready，数据库与 Redis 均为 ok。
- QQ 邮箱真实登录邮件投递成功；管理员魔法链接登录成功，邀请状态为 accepted。
- ChatGPT、Grok、DeepSeek 模型连接卡片正常，未配置状态清晰；Skill 设置入口、内置 Skill 和单次创作多选正常。
- 生产环境未配置默认模型时返回 `AI_NOT_CONFIGURED`（HTTP 422），不会创建 `local-template` 内容；检查前后本地模板内容数均为 0。
- 新发布构建时间：2026-07-15T18:25:21Z 至 2026-07-15T18:27:22Z；服务器构建、目标回归测试、迁移状态和 11/11 预检均通过。

## 回滚与已知限制

- DNS 回滚/稳定态：A 记录保持或恢复为 `65.49.231.53`。
